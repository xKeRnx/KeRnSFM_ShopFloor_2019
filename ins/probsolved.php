...
if (isset($_SESSION['id'])){
	...
	if ($result0->num_rows == 1) {
		...
		if($projsolution != ""){
			if($projcreator == $suid AND $projstatus1 != 0 OR $isAdm == 1 AND $projstatus1 != 0){
				$sql = "UPDATE project_entry SET status='0', solvedat=Now() WHERE id='$project'";
				$result = $conn->query($sql);

				$commmsg = base64_encode('has closed Problem');
				// entry_type -> 0 = escalated / 1 = add solution / 2 = new Solution time / 3 = Problem solved
				$sql1 = "INSERT INTO entry_history(entry, message, creator, project_owner, project_creator, entry_type) VALUES('$project', '$commmsg', '$suid', '$projowner', '$projcreator', 3)";
